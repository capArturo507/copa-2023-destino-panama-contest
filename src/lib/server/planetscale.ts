...
export const requestStoredData = (queryParams: DataBase.Participante) =>
	planetscale.execute(
		'SELECT id, instagram, email, phone, questions, correct_answers, completed_time_ms FROM participants WHERE id=:id OR instagram=:instagram OR email=:email OR phone=:phone;',
		queryParams
	);

export const participate = (queryParams: any) =>
	planetscale.execute(
		'UPDATE participants SET answers=:answers, completed_datetime=:completedDateTime, correct_answers=:correctAnswers, completed_time_ms=:completedTimeMS;',
		queryParams
	);
